...
try:
    ...
    ROLE_ALIAS_NAME = os.environ['ROLE_ALIAS_NAME']
    ROLE_ALIAS_ROLE_ARN = os.environ['ROLE_ALIAS_ROLE_ARN']
...
def create(event, context):
    ...
    client.create_policy(
        policyName='{}-{}'.format(thingName, stackName),
    ...
    logger.info('Created thing: %s, cert: %s and policy: %s' % 
        (thingName, certId, '{}-{}'.format(thingName, stackName)))

    iotEndpoint = client.describe_endpoint(endpointType='iot:Data-ATS')['endpointAddress']

    iotCredentialsEndpoint = client.describe_endpoint(endpointType='iot:CredentialProvider')['endpointAddress']
    ...
    helper.Data.update({"IoTEndpointCredentialProvider": iotCredentialsEndpoint})
