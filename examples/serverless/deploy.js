...
const output=spawnOrFail('sam', ['deploy',
...
const invokeUrl=spawnOrFail('aws', ['cloudformation',
                    'describe-stacks', '--stack-name', `${stack}`,
                    '--query', 'Stacks[0].Outputs[0].OutputValue',
                    '--output', 'text',
                    '--region', `${region}`]);
console.log(`Streaming API Gateway invoke URL: ${invokeUrl}`);

const ecsClusterName=spawnOrFail('aws', ['cloudformation', 'describe-stacks',
                    '--stack-name', `${stack}`,
